document.addEventListener('DOMContentLoaded', () => {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', (e) => {
            ...
            // 1. Vérification du Honeypot (Pot de miel anti-bot)
            const honeypot = contactForm.querySelector('#website');
            if (honeypot && honeypot.value !== '') {
                // C'est un bot ! On ne fait pas le fetch mais on simule un succès pour le tromper
                console.log("Activité suspecte détectée, soumission ignorée silencieusement.");
                setTimeout(() => {
                ...
                return; // Arrêt total de la fonction, la requête N8N n'est pas envoyée !
            }
